...
echo "🚀 覆盖飞书文档..."
# Some lark-cli versions do not expand @file for docs +update; pass the
# rendered Markdown content explicitly to avoid writing the literal path.
MARKDOWN_CONTENT="$(cat "$TEMP_MD")"
CMD=(lark-cli docs +update --doc "$FEISHU_DOC_URL" --mode overwrite --markdown "$MARKDOWN_CONTENT")
if [ -n "$NEW_TITLE" ]; then
